...
| ↳ phase 6 · **contribution meadow** 🌾 | The weekly growth log unrolled as luminous grass on the ground disc (guarded on `tree.json`'s `contribution` array — older data renders unchanged). Each week scatters `level` **blades** in its sector's wedge: **height = the week's contribution count**, **radius = when** (recent weeks ring the **rim**, so the meadow reads like tree rings), **hue = the sector**, paled toward **moonlit blue** by the private share of that week's weight — aggregates only, never ids. The bottom **timeline strip** is the same ring unrolled (per-week columns, weight-weighted hues, same private desaturation). Interactions: **hover a blade** for its week panel (`Week of … · sector · N contributions · P public · Q private · weight W`); a soft-cyan **time-cursor ring** tracks the growth front while you scrub or replay (with a replay pulse); a **click on the ground** selects that sector's wedge (Sector Walk — the timeline slider is the time interface); and **hovering a leaf cluster** flashes its sector's blades of the same week (canopy↔ground). Extra `__dbg`: `pickMeadow`, `showMeadow`, `setMeadowFlash`. Renders in **both** the mood sketch and the acacia reference sketch (ported so the canonical viewer carries it). |

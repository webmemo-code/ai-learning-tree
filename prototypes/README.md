...
| [grove-walk/](grove-walk/) | Phase-6 build: the **walkable 3D grove** ([docs/05-grove.md](../docs/05-grove.md) G5). The same mock grove in first person — **drag** to look, **WASD/arrows** to walk, **wheel** to glide; a **🚶 Stroll** mode (on by default, any input cancels) wanders among each clearing's trees and *flies* the empty meadow between clearings, where the phyllotaxis pattern reads from the air. Every member renders as an **impostor**: one instanced-billboard draw for all crowns, one instanced trunk mesh — the two *real* members (`webmemo-code/ai-learning-tree`, the mock fixture) build their impostors from their fetched `tree.json` (leaf-cluster centers/radii, sector hues + recency, blossom positions); mock members get seeded plausible specs. Click a tree → member id, clearing, slot, planted date, and a "visit this tree →" link to its own full renderer. Stumps stay as bark-hued rings. Supports `?dpr=` (+ adaptive quality), `?hud=0`, `?stroll=0`. `__dbg`: `goTo(id)`, `pickAt`, `setStroll`, `state`. |
